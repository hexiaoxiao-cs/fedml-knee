...
scores=pd.DataFrame()
Model_path="/research/cbim/vast/xh172/FedCV/experiments/distributed/segmentation_knee/run/OAI_semi_did/semi_OAI/OAI_SEMI_with_did_epoch_20_4_machine/model_best.pth.tar"
# 1. Get Images:
root_dir = '/research/cbim/vast/xh172/FedCV/data/OAI'
list_label="label_file_did.txt"
list_bl="bl_sag_3d_dess_with_did.txt"
list_12m="12m_sag_3d_dess_with_did.txt"
...
for i in range(4):
    ...
    print("label_train",len(label_train[i]))
    label_val+=label[i][tra[i]:tra[i]+int((len(label[i])-tra[i])/2)]
    print("label_val",len(label_train))
    print(int((len(label[i])-tra[i])/2))
...
model = VNet(model_channel=net_channel,
                     out_class=4)
model.load_state_dict(torch.load(Model_path)['state_dict'])
model.cpu()
model.eval()
save_path = 'OAI_with_did_Only_sensetime_label/'

for subject_name,path_i,i in label_test:
    id = subject_name
    print(subject_name)
    print(path_i)
    print(i)
    ...
    x=torch.from_numpy(fixed_size_img[np.newaxis, :][np.newaxis, :])
    ...
    pred_itk.SetOrigin(img_sitk.GetOrigin())
    pred_itk.SetSpacing(spacing_c_a_s)
    pred_itk.SetDirection(img_sitk.GetDirection())
    ...
    resampled_label_itk = itk_resample_only_label(pred_itk, out_spacing=img_sitk.GetSpacing())

    pred = sitk.GetArrayFromImage(resampled_label_itk)

    # pad or crop to origin size
    pred = pad_crop(pred, img_sitk.GetSize()[::-1], mode='constant',
                    value=0)
    pred_itk = sitk.GetImageFromArray(pred)
    pred_itk.SetOrigin(img_sitk.GetOrigin())
    pred_itk.SetSpacing(img_sitk.GetSpacing())
    pred_itk.SetDirection(img_sitk.GetDirection())
    ...
    sitk.WriteImage(pred_itk, save_path + id + '_pred.mha')
    sitk.WriteImage(img_sitk,save_path+id+"_img.mha")
    sitk.WriteImage(label_sitk,save_path+id+"_lbl.mha")
