...
model.cpu()
model.eval()
save_path = 'BB_prediction/'


for i in tqdm.tqdm(range(len(list_validation))):
    image = np.asarray(Image.open(list_validation[i]))
    image = pad_crop(image,(256,256))
    to_save_img=np.copy(image)
    image = (image - image.mean()) / image.std()
    
    label = np.asarray(Image.open(list_validation[i].replace("_img","_label")))
    to_save_lbl=np.copy(label)
    ...
    to_save_lbl_i=Image.fromarray(to_save_lbl)
    to_save_lbl_i=to_save_img_i.convert("L")
