...
#Model_path="/research/cbim/vast/xh172/FedCV/experiments/distributed/segmentation_knee/run/cardiac_MnM_3D/semi_OAI_distil/MnM3D_our_method_e30_c30_3cli_b32_u16_new/model_best.pth.tar"
Model_path="/research/cbim/vast/xh172/FedCV/experiments/distributed/segmentation_knee/run/cardiac_MnM_3D/semi_OAI_distil/MnM3D_our_method_e30_c30_3cli_b32_u16_new/experiment_3/checkpoint.pth.tar"
file_root="/filer/tmp1/xh172/MM_challenge"
list_validation=glob.glob(os.path.join(file_root,"testing_3D_only_two_new","*_sa_t*"))
...
model = VNet(model_channel=1,
                     out_class=4)
model.load_state_dict(torch.load(Model_path)['state_dict'])
model.cpu()
model.eval()
save_path = 'MnM_3D_with_our_method/'


for i in tqdm.tqdm(range(len(list_validation))):
    bn=os.path.basename(list_validation[i])
    bn=os.path.splitext(bn)[0]
    image_sitk=sitk.ReadImage(list_validation[i])
    image=sitk.GetArrayFromImage(image_sitk)
    image=(image - image.mean()) / image.std()
    label_sitk = sitk.ReadImage(list_validation[i].replace("_sa_t","_sa__gt_t"))
    ...
    pred_itk.SetOrigin(image_sitk.GetOrigin())
    pred_itk.SetSpacing(image_sitk.GetSpacing())
    pred_itk.SetDirection(image_sitk.GetDirection())
    ...
    # truth_2 = truth == 2
    # truth_truth = truth > 0
    voxelspacing = list(image_sitk.GetSpacing()).reverse()
    line=list()
    line.append(bn)
    ...
    sitk.WriteImage(pred_itk, save_path + bn + '_pred.nii.gz')
    sitk.WriteImage(image_sitk,save_path+bn+"_img.nii.gz")
    sitk.WriteImage(label_sitk,save_path+bn+"_lbl.nii.gz")
