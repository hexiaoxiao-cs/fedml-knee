...
lbl_sitk_l=[]
# 0.Read image files
path_case = '../../../data/sensetime/list_testing.txt'
...
#Model_path="/research/cbim/vast/xh172/FedCV/experiments/distributed/segmentation_knee/run/OAI_semi_did/semi_OAI/OAI_semi_reduced_bs_4/model_best.pth.tar"
#Model_path="/research/cbim/vast/xh172/FedCV/experiments/distributed/segmentation_knee/run/cardiac_biobank/cardiac_semi/biobank_3machine/model_best.pth.tar"
Model_path="/research/cbim/vast/xh172/FedCV/experiments/distributed/segmentation_knee/run/cardiac_MnM/cardiac_semi_distil/MnM2_our_method_e30_c30_3cli_b32_u16/model_best.pth.tar"
# with open(path_case, 'r') as f:
#     for i in f.readlines():
#         path_i = i.split("\n")[0]
#         file_path.append(os.path.join(base_path, path_i))
# for i in glob.glob("/research/cbim/vast/xh172/FedCV/data/sensetime/cropped_img_labeled/*/org.mha"):
#     file_path.append(i)
file_root="/filer/tmp1/xh172/MM_challenge"
list_validation=glob.glob(os.path.join(file_root,"testing_png_only_two_new","*_sa_t*"))

# f=open("/research/cbim/vast/xh172/UKBiobank-Processing/validation_img_lbl.txt","r")
...
model.cpu()
model.eval()
save_path = 'MnM_with_our_model_3/'


for i in tqdm.tqdm(range(len(list_validation))):
    image = np.asarray(Image.open(list_validation[i]))
    # image = pad_crop(image,(256,256))
    to_save_img=np.copy(image).astype(np.uint8)
    # print(to_save_img.shape)
    image = (image - image.mean()) / image.std()
    label = np.asarray(Image.open(list_validation[i].replace("_sa_t","_sa__gt_t")))
    to_save_lbl=np.copy(label).astype(np.uint8)
    # for c, k in enumerate(np.unique(label)):
    #     label[label==k]=c
    label = pad_crop(label,(256,256))
    ...
    to_save_img_i=Image.fromarray(to_save_img)
    to_save_img_i=to_save_img_i.convert("L")
    to_save_lbl_i=Image.fromarray(to_save_lbl.astype(np.uint8)/2*255)
    to_save_lbl_i=to_save_lbl_i.convert("L")
    to_save_pred_i=Image.fromarray(pred.astype(np.uint8)/2*255)
    to_save_pred_i=to_save_pred_i.convert("L")
    idd=os.path.basename(list_validation[i])
    to_save_img_i.save(os.path.join(save_path,idd+"_img.png"))
    to_save_lbl_i.save(os.path.join(save_path,idd+"_lbl.png"))
    to_save_pred_i.save(os.path.join(save_path,idd+"_pred.png"))
