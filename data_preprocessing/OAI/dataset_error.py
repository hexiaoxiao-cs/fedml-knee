...
class OAI_data(data.Dataset):
    def __init__(self,list_file,root_dir='../../../data/oai',sagittal_num=32, transforms=None, bd=False):
        self.image = []
        self.label = []
        self.path = []
        self.idd = []
...
class OAI_Data_non_label(data.Dataset):
    def __init__(self,list_file,root_dir='../../../data/oai',data_root='bl',sagittal_num=32, transforms=None, bd=False):
        self.image = []
        self.path = []
        self.idd = []
        self.transforms = transforms
        ...
        self.label_path=[]
        self.file_path=[]
        self.temp_zero = np.zeros((sagittal_num, 512, 512)).astype(np.uint8)
        cnt = 0
        # self.inform = []
        # self.norm_empty_slice_list = []
        # self.slices_list = [0]
        ...
        for i in self.file_path:
            ...
            sagital_num_slices = sagittal_num # resample_img.shape[0]  ###################### notice
            # if isSWIN==True: sagital_num_slices = 32
            fixed_size_img = pad_crop(resample_img, (sagital_num_slices, 512, 512), mode='constant', value=0)            #### move the menis labels(3,4) to (7,8), not necessary
            #### z-score norm
            fixed_size_img = (fixed_size_img - fixed_size_img.mean()) / fixed_size_img.std()
...
class OAI_Semi(data.Dataset):
    def __init__(self,list_label,list_unlabel):
        self.image=[]
        self.label=[]
        with open(list_label,"r"):
            


class MyData_SEMI(data.Dataset):
    def __init__(self,list_label,list_bl,list_12m,root_dir='../../../data/oai',sagittal_num=32):
...
if __name__=='__main__':
    ...
    train_data = MyData_SEMI("../../data/OAI/train.txt","../../data/OAI/bl_sag_3d_dess_leftRight.txt","../../data/OAI/12m_sag_3d_dess_leftRight.txt",root_dir="../../data/OAI")
