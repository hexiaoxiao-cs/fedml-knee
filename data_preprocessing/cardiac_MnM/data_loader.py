...
def partition_data_semi_with_did():
    ...
    for i in glob.glob(os.path.join(file_root,"labeled_png_new","*_sa_t*.png")):
        bn=os.path.basename(i)
        id=bn.split("_")[0]
        labeled_dict[int(df[df["External code"]==id]["Centre"])].append(i)
    for i in glob.glob(os.path.join(file_root,"unlabeled_png_new","*_sa_t*.png")):
        bn=os.path.basename(i)
        id=bn.split("_")[0]
    ...
    val_ds=MnM_data(glob.glob(os.path.join(file_root,"validation_png_only_two_new","*_sa_t*")),None)
    train_label=0
    train_unlabel=0
    for i in train_ds:
...
def load_partition_data_MnM_with_did(client_number, batch_size, batch_size_lb,other_num=256):
