...
class MnM_data(data.Dataset):
    def __init__(self,labeled_list,unlabeled_list,is_val=False,other_num=256, transforms=None, bd=False):
        # self.image = []
        self.image_labeled=[]
        self.is_val=is_val
        ...
        for i in labeled_list:
            ...
            self.file_path_labeled_label.append(i.replace("_sa_t","_sa__gt_t"))
        self.file_path_unlabeled=unlabeled_list
        #Determine whether the file is labeled or unlabeled by checking ED and ES are both 0 (in unlabeled folder)
        #Assume that master list is something like 
        #    [[path_1,site,l1,l2],[path_2,site,l1,l2]...]
        # for i in master_list:
        #     img=sitk.ReadImage(i[0])
        #     img_np=sitk.GetArrayFromImage(img)
        #     if "Unlabeled" in i[0]:
        #         shifted_img=img_np.reshape(img_np.shape[0]*img_np.shape[1],img_np.shape[2],img_np.shape[3])
        #         padded_img=pad_crop(shifted_img,(shifted_img.shape[0],256,256))
        #         self.image_unlabeled.extend(padded_img.tolist())
        #         continue
        #     else:
        #         lbl=sitk.ReadImage(i[0].replace("_sa","_sa_gt"))
        #         lbl_np=sitk.GetArrayFromImage(lbl)
        #         padded_img=pad_crop(img_np,(img_np.shape[0],img_np.shape[1],256,256))
        #         padded_lbl=pad_crop(lbl_np,(lbl_np.shape[0],lbl_np.shape[1],256,256))
        #         da=max(int(i[2]),int(i[3]))
        #         xiao=min(int(i[2]),int(i[3]))
        #         p1=padded_img[:xiao]
        #         p2 = padded_img[xiao+1:da]
        #         p3=padded_img[da+1:]
        #         shifted_p1=p1.reshape(p1.shape[0]*p1.shape[1],p1.shape[2],p1.shape[3])
        #         shifted_p2=p2.reshape(p2.shape[0]*p2.shape[1],p2.shape[2],p2.shape[3])
        #         shifted_p3=p3.reshape(p3.shape[0]*p3.shape[1],p3.shape[2],p3.shape[3])
        #         self.image_unlabeled.extend(shifted_p1.tolist())
        #         self.image_unlabeled.extend(shifted_p2.tolist())
        #         self.image_unlabeled.extend(shifted_p3.tolist())
        #         l1=padded_img[xiao]
        #         l2=padded_img[da]
        #         l1_lbl=padded_lbl[xiao]
        #         l2_lbl=padded_lbl[da]
        #         self.image_labeled.extend(l1.tolist())
        #         self.image_labeled.extend(l2.tolist())
        #         self.label.extend(l1_lbl.tolist())
        #         self.label.extend(l2_lbl.tolist())
                # for j in range(padded_img.shape[0]):
                #     if j == i[2] or j==i[3]:
                #         for k in range(padded_img[j].shape[0]):
                #             self.image_labeled.append(padded_img[j][k])
                #             self.label.append(padded_lbl[j][k])
                #     else:
                #         for k in range(padded_img[j].shape[0]):
                #             self.image_unlabeled.append(padded_img[j][k])
                # shifted_img=img_np.reshape(img_np.shape[0]*img_np.shape[1],img_np.shape[2],img_np.shape[3])
                # padded_img=pad_crop(shifted_img,(shifted_img.shape[0],256,256))
                # shifted_img=img_np.reshape(img_np.shape[0]*img_np.shape[1],img_np.shape[2],img_np.shape[3])
                # padded_img=pad_crop(shifted_img,(shifted_img.shape[0],256,256))

        # for i in glob.glob(os.path.join(root_dir,"Training","Labeled","*")):
        #     bn=os.path.basename(i)
        #     img_p = os.path.join(i,bn+"_sa.nii.gz")
        #     gt_p=os.path.join(i,bn+"_sa_gt.nii.gz")
        #     img=sitk.ReadImage(img_p)
        #     gt=sitk.ReadImage(gt_p)
        #     img_np=sitk.GetArrayFromImage(img)
    ...
    def __getitem__(self, index):
        ...
        # image = np.asarray(Image.open(path_i))
        # image = pad_crop(image,(self.other_num,self.other_num))
        # if mode == 1:
        #     label=self.temp_zero
        # else:
        #     label = np.asarray(Image.open(path_label))
        #     label = pad_crop(label,(self.other_num,self.other_num))
        # image = (image - image.mean()) / image.std()
        # for c, i in enumerate(np.unique(label)):
        #     label[label==i]=c

        if index<self.num_labeled:
            image=sitk.ReadImage(self.file_path_labeled[index])
            label=sitk.ReadImage(self.file_path_labeled_label[index])
            image=sitk.GetArrayFromImage(image)
            label=sitk.GetArrayFromImage(label)
            # image=np.asarray(Image.open(self.file_path_labeled[index]))
            # label=np.asarray(Image.open(self.file_path_labeled_label[index]))
            # print(np.unique(label))
            # First we clip the 99.5% and 0.5% intensity value
            image=np.clip(image,np.percentile(image,0.5),np.percentile(image,99.5))
            #Here we normalize the image through the Z-score
            image = (image - image.mean()) / image.std()
            # for c, i in enumerate(np.unique(label)):
            #     label[label==i]=c
            return image[np.newaxis, :], label.astype(np.uint8)
        else:
            image=sitk.ReadImage(self.file_path_unlabeled[index-self.num_labeled])
            image=sitk.GetArrayFromImage(image)
            image=np.clip(image,np.percentile(image,0.5),np.percentile(image,99.5))
            # image=np.asarray(Image.open(self.file_path_unlabeled[index-self.num_labeled]))
            label=self.temp_zero
            image = (image - image.mean()) / image.std()
            return image[np.newaxis,:],label.astype(np.uint8)
    ...
    def __repr__(self):
        if self.is_val==True:
            return 'cases: ' + str(self.num_labeled)
        else:
            return 'cases: ' + str(self.num_labeled+self.num_unlabeled)
...
if __name__=='__main__':
    f=open("/research/cbim/vast/xh172/UKBiobank-Processing/training_img_lbl.txt","r")
    ll=f.read().splitlines()
    f.close()
    f=open("/research/cbim/vast/xh172/UKBiobank-Processing/training.txt","r")
    lu=f.read().splitlines()
    f.close()
    d=collections.defaultdict(list)
    for i in ll:
        stuff=i.split("\t")
        # print(stuff[0].replace("\t",""))
        d[int(stuff[1])].append(stuff[0].replace("\t","").replace(" ",""))
    
    ds=BB_data(d[0],lu)
    print(ds.num_labeled)
    print(ds.num_unlabeled)