...
def partition_data_semi_with_did():
    # master_list=collections.defaultdict(list)
    file_root="/filer/tmp1/xh172/MM_challenge"
    train_ds=[]
    # f = open("/research/cbim/vast/xh172/UKBiobank-Processing/mm_training_labeled.txt","r")
    # list_l=f.read().splitlines()
    # f.close()
    df=pd.read_csv("/filer/tmp1/xh172/MM_challenge/info.csv")
    labeled_dict=collections.defaultdict(list)
    unlabeled_dict=collections.defaultdict(list)
    for i in glob.glob(os.path.join(file_root,"labeled_3d_new","*_sa_t*.nii.gz")):
        bn=os.path.basename(i)
        id=bn.split("_")[0]
        labeled_dict[int(df[df["External code"]==id]["Centre"])].append(i)
    # for i in glob.glob(os.path.join(file_root,"labeled_3d_new","*_sa__gt_t*.nii.gz")):
    #     bn=os.path.basename(i)
    #     id=bn.split("_")[0]
    #     unlabeled_dict[int(df[df["External code"]==id]["Centre"])].append(i)
    for i in glob.glob(os.path.join(file_root,"unlabeled_3d_new","*_sa_t*.nii.gz")):
        bn=os.path.basename(i)
        id=bn.split("_")[0]
        unlabeled_dict[int(df[df["External code"]==id]["Centre"])].append(i)
    for i in range(1,6):
        if i == 4: continue
        train_ds.append(MnM_data(labeled_dict[i],unlabeled_dict[i]))
        
    # for i in list_l:
    #     l_i=i.split(" ")
    #     for j in l_i:
    #         j.replace(" ","")
    #     master_list[int(l_i[1])].append(l_i)
    # train_ds=[]
    # ml=list(master_list.values())
    # # print(ml)
    # my_pool=Pool(6)
    # train_ds=my_pool.map(get_DL,ml)
    # my_pool.close()
    # my_pool.join()
    # print("Done")
    # for i in range(1,6):
    #     train_ds.append(MnM_data(master_list[i]))
    # f = open("/research/cbim/vast/xh172/UKBiobank-Processing/mm_validation.txt","r")
    # list_l = f.read().splitlines()
    # validation=[]
    # for i in list_l:
    #     l_i=i.split(" ")
    #     for j in l_i:
    #         j.replace(" ","")
    #     validation.append(l_i)
    val_ds=MnM_data(glob.glob(os.path.join(file_root,"validation_3D_only_two_new","*_sa_t*")),None)
    train_label=0
    train_unlabel=0
    for i in train_ds:
        train_label=train_label+i.num_labeled
        train_unlabel=train_unlabel+i.num_unlabeled
    # f.close()
    val_size=val_ds.image_labeled
...
#         s=i.split("\t")
#         labeled_train[int(s[1])].append(s[0].replace("\t","").replace(" ",""))
#     f = open(file_unlabeled,"r")
#     list_unlabeled=f.read().splitlines()
#     f.close()
#     for i in list_unlabeled:
#         s=i.split("\t")
#         unlabeled_train[int(s[1])].append(s[0].replace("\t","").replace(" ",""))
#     train_ds=[]
...
#     f.close()
#     f=open("/research/cbim/vast/xh172/UKBiobank-Processing/testing_img_lbl.txt","r")
...
#     return train_ds,test_ds,train_label,train_unlabel,val_size
...
def load_partition_data_MnM_3D_with_did(client_number, batch_size, batch_size_lb,other_num=256):
    ...
    # list_bl="bl_ST.txt"
    # list_12m="12m_ST.txt"
    # list_unlabel="/research/cbim/vast/xh172/UKBiobank-Processing/training_with_grp.txt"
    # list_label="/research/cbim/vast/xh172/UKBiobank-Processing/training_img_lbl.txt"
    # training_did=[0,1,2]
    
    train_ds,test_ds,train_label,train_unlabel,test_size = partition_data_semi_with_did()
    train_data_num=train_label+train_unlabel
    ...
    def worker_init_fn(worker_id):
        random.seed(10 + worker_id)
    train_ds_global=ConcatDataset(train_ds)
    train_data_global=DataLoader(dataset=train_ds_global, batch_sampler=sampler, num_workers=32, pin_memory=True,worker_init_fn=worker_init_fn)
    test_data_global = DataLoader(dataset=test_ds,batch_size=batch_size,shuffle=False, drop_last=False,num_workers=32,pin_memory=True)
    test_data_num = len(test_ds)
    # get local dataset
    data_local_num_dict = dict()  # Number of samples for each client
    train_data_local_dict = dict()
    ...
    for client_idx in range(client_number):
        ...
        data_local_num_dict[client_idx] = local_data_num
        # print(local_data_labeled_num,local_data_unlabeled_num)
        # train_dataset_local=Subset(train_ds,data_idxs)
        sampler=TwoStreamBatchSampler(range(local_data_labeled_num),range(local_data_labeled_num,local_data_unlabeled_num+local_data_labeled_num),batch_size,batch_size-batch_size_lb)
        def worker_init_fn(worker_id):
            random.seed(10+ worker_id)
...
if __name__=="__main__":
    ...
    train_data_local_dict, test_data_local_dict, num=load_partition_data_MnM_with_did(3,4,2,other_num=256) #client_number, batch_size, batch_size_lb,sagittal_num)
    print(train_data_num)
    print(test_data_num)
