...
class FedSegAggregator(object):
    ...
    def get_global_ema_params(self):
        return self.trainer.get_ema_params()

    def set_global_model_params(self, model_parameters):
        self.trainer.set_model_params(model_parameters)
    def set_global_ema_params(self,ema_params):
        self.trainer.set_ema_params(ema_params)

    def add_local_trained_result(self, index, model_params,ema_params, sample_num):
        logging.info("Add model index: {}".format(index))
        self.model_dict[index] = model_params
        self.ema_dict[index]=ema_params
    ...
    def aggregate_ema(self):
        ...
        for idx in range(self.worker_num):
            if self.args.is_mobile == 1:
                self.ema_dict[idx] = transform_list_to_tensor(self.ema_dict[idx])
            model_list.append((self.sample_num_dict[idx], self.ema_dict[idx]))
            training_num += self.sample_num_dict[idx]

        logging.info("Aggregating...... {0}, {1}".format(len(self.ema_dict),len(model_list)))
        ...
        # update the global model which is cached at the server side
        self.set_global_ema_params(averaged_params)
    ...
    def add_client_test_result(self, round_idx, client_idx, train_eval_metrics:EvaluationMetricsKeeper, test_eval_metrics:EvaluationMetricsKeeper):
        ...
        if self.args.save_client_model:
            best_mIoU = self.best_mIoU_clients.setdefault(client_idx, 10000.)
            ...
            if test_mIoU < best_mIoU:
                ...
                saver_state = {
                    ...
                    'state_dict_ema': self.ema_dict[client_idx]   
                }
    ...
    def output_global_acc_and_loss(self, round_idx):
        ...
        if test_mIoU < self.best_mIoU:
            ...
            if self.args.save_model:
                ...
                is_best = True
                if "semi" in self.args.model:
                    saver_state = {
                        'best_pred': self.best_mIoU,
                        'round': round_idx + 1,
                        'state_dict': self.trainer.get_model_params(),
                        'state_dict_ema' : self.trainer.get_ema_params(),
                    }
                else:
                    saver_state = {
                        'best_pred': self.best_mIoU,
