...
class FedSegClientManager(ClientManager):
    ...
    def handle_message_init(self, msg_params):
        ...
        global_ema_params=msg_params.get(MyMessage.MSG_ARG_KEY_EMA_PARAMS)
        ...
        self.trainer.update_ema_model(global_ema_params)
    ...
    def handle_message_receive_model_from_server(self, msg_params):
        ...
        if self.args.aggregate_net=="both":
            self.trainer.update_ema_model(ema_params)
            logging.info("Updating Received EMA model/teacher")
        self.trainer.update_dataset(int(client_index))
        self.round_idx += 1
        self.__train()
    ...
    def send_model_to_server(self, receive_id, weights,ema_weights, local_sample_num, train_evaluation_metrics, test_evaluation_metrics):
        message = Message(MyMessage.MSG_TYPE_C2S_SEND_MODEL_TO_SERVER, self.get_sender_id(), receive_id)
        message.add_params(MyMessage.MSG_ARG_KEY_MODEL_PARAMS, weights)
        message.add_params(MyMessage.MSG_ARG_KEY_EMA_PARAMS,ema_weights)
    ...
    def __train(self):
        ...
        weights, ema_model,local_sample_num = self.trainer.train()
        self.send_model_to_server(0, weights, ema_model, local_sample_num, train_evaluation_metrics, test_evaluation_metrics)
