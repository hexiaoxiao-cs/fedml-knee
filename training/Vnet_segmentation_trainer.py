...
class SegmentationTrainer(ModelTrainer):
    ...
    def train(self, train_data, device):   
        ...
        #self.weight_dice = torch.FloatTensor([1, 1, 1, 1, 1, 1, 1,1,1]).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            for (batch_idx, batch) in enumerate(train_data):
                ...
                loss = WeightedDiceLoss(pred, labels.long(), self.weight_dice)
                # preds = torch.zeros([stride * T, 9, 32, 512, 512]).to(device)
                ...
                # #logging.info("preds size"+str(preds.shape))
                #
                # preds = preds.reshape(T, stride, 9, 32, 512, 512)
                # #logging.info("preds size"+str(preds.shape))
                #
