    ...
    def train(self, train_data, device):   
        ...
        for epoch in range(args.epochs):
            ...
            for (batch_idx, batch) in enumerate(train_data):
                ...
                labeled_volume_batch=x[:args.batch_size_lb]
                noise = torch.clamp(torch.randn_like(unlabeled_volume_batch) * beta, -beta, beta)
                noise_strong=torch.clamp(torch.randn_like(unlabeled_volume_batch) * (1.0-beta), -(1.0-beta), (1.0-beta))
                unlabeled_weak = unlabeled_volume_batch + noise
                unlabeled_strong=unlabeled_volume_batch+noise_strong
                inputs = torch.cat((labeled_volume_batch, unlabeled_weak,unlabeled_strong)).to(device)
                outputs=model(inputs.float())
                # logits_x = model(unlabeled_volume_batch.float().to(device))
                # logits_u_w=model(unlabeled_weak.float().to(device))
                # logits_u_s=model(unlabeled_strong.float().to(device))
                logits_x = outputs[:args.batch_size_lb]
                # # logits_u_w = outputs[args.batch_size_lb:]
                logits_u_w=outputs[args.batch_size_lb:args.batch_size_lb*2]
                logits_u_s=outputs[args.batch_size_lb*2:]
